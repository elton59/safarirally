   ...
   if(isset($_GET['apapsid']))
   {
    $em=$_GET['apapsid'];
        ...
        $result=$mysqli->query("select * from eventdetail where status='approved'")or die($mysqli->error);
                  $result2=$mysqli->query("select * from eventdetail where   payment_status!='nopay'")or die($mysqli->error);
...
<h3 align="center"><u>Events Yet To be paid For</u></h3>
...
$pdf->output('Events Yet To be paid For.pdf','I');
