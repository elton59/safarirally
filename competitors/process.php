...
if (isset($_POST['login'])) {
  ...
  $result = $mysqli->query("SELECT * FROM driverdetails where email='$email' AND password='$password' and status='approved'") or die($mysqli->error);
...
if (isset($_POST['uadr'])) {
  ...
  $sql = $mysqli->query(" UPDATE driverdetails SET national_id='$drid',fullname='$drname',lincensenumber='$drlno',nationality='$drnat',codriverid='$drcodid',sponsorid='$drspon',email='$dremail',password='$drpass' where id='$id'") or die($mysqli->error);
  if ($sql) {
...
if (isset($_GET['bpid2'])) {
  $id = $_GET['bpid2'];
  $sql = $mysqli->query("update  eventdetail  set status='paid_awaiting_approval' where id='$id'") or die($mysqli->error);

  if ($sql) {

    echo "<script>alert(' success');
   window.location.replace('createpayments.php');
...
if (isset($_POST['adcod'])) {
  ...
  if ($sql) {
    ...
    echo "<script>alert(' success');
       window.location.replace('viewcodrivers.php');

           </script>";
  } else {
    echo "<script><alert>(' failed');
           window.location.replace('editcodrivers.php');
           </script>";
  }
}
...
if (isset($_POST['uadcod'])) {
  ...
  $sql = $mysqli->query("UPDATE codriverdetails SET codriverid='$codid',fullname='$codname',teamid='$codtid',nationality='$codnat',gender='$codgen',email='$codemail' where id=$id") or die($mysqli->error);
  if ($sql) {
...
//delete codriver
if (isset($_GET['delcodid'])) {
  $id = $_GET['delcodid'];
  $sql = $mysqli->query("delete from codriverdetails where id='$id'") or die($mysqli->error);
...
if (isset($_POST['adpay'])) {
  ...
  if ($sql) {
    ...
    echo "<script><alert>(' failed');
           window.location.replace('editpayments.php');
           </script>";
  }
}
//edit payments

if (isset($_POST['uadpay'])) {
  $id = $_POST['id'];
  $ptrid = $_POST['ptrid'];
  $pname = $_POST['pname'];
  $ppf = $_POST['ppf'];
  ...
  $sql = $mysqli->query("UPDATE paymentdetails SET transactiontid='$ptrid',accountname='$pname',amountpaid='$pamount',paymentfor='$ppf',usercategory='$puc' where id='$id'") or die($mysqli->error);
  if ($sql) {
...
if (isset($_POST['adrcar'])) {
  ...
  if ($sql) {
    ...
    echo "<script>alert('record added successfully');
       window.location.replace('viewracecar.php');
...
if (isset($_POST['adrecar'])) {
  ...
  if ($sql) {
    ...
    echo "<script>alert('record added successfully');
       window.location.replace('viewracee.php');
