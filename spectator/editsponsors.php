  ...
  <section id="container" class="">
    ...
    <section id="main-content">
      <section class="wrapper">
        <div class="row">
          <div class="col-lg-12">
            <h3 class="page-header"><i class="fa fa-table"></i> Edit Sponsors</h3>
            <ol class="breadcrumb">
              ...
              <table class="table-responsive table table-bordered">
                ...
                <?php
                  ...
                  while($row=$result->fetch_assoc())
                  {
                    ...
                    <td>".$row['email']."</td>
                    <td>".$row['password']."</td>
                    <td> <a href='editsponsors.php?apspid=$row[id]' class='btn btn-primary' >Edit<a>
                   <a href='deletesponsors.php? delspid=$row[id]' class='btn btn-danger'>Delete<a></td>
                   </tbody>
                    "
                  ;}
            ?>
              ...
              <?php
    if (isset($_GET['apspid']))
{
  $id=$_GET['apspid'];
  $result=$mysqli->query("select * from sponsordetails where id=$id")or die($mysqli->error);
  $row=$result->fetch_array();
   $spid= $row['sponsorid'];
   $spname=$row['sponsorname'];
   $sptid=$row['teamid'];
   $spemail=$row['email'];
   $sppass=$row['password'];
...
?>
 ...
 <form class="form-horizontal " method="POST" action="process.php">
                  ...
                  <input  name="id"type="hidden" class="form-control round-input"  value='<?php echo $id; ?>'>
                    ...
                    <div class="col-sm-10">
                      <input  name="spid"type="text" class="form-control round-input" placeholder="input Sponsor ID" value='<?php echo $spid; ?>'>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Sponsor Name</label>
                    <div class="col-sm-10">
                      <input name="spname" type="text" class="form-control round-input" placeholder="input Sponsor Name" value='<?php echo $spname; ?>'>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Team ID</label>
                    <div class="col-sm-10">
                      <input name="sptid" type="text" class="form-control round-input" placeholder="input Driver Team ID" value='<?php echo $sptid; ?>'>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Email</label>
                    <div class="col-sm-10">
                      <input name="spemail" type="text" class="form-control round-input" placeholder="input Driver Email" value='<?php echo $spemail; ?>'>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label">Password</label>
                    <div class="col-sm-10">
                      <input name="sppass" type="text" class="form-control round-input" placeholder="input Password"  value='<?php echo $sppass; ?>'>
                    </div>
                  </div>
                 
                
                  <center>
                  <input class="btn btn-primary" href="" title="Bootstrap 3 themes generator" type="submit" name="uadsp" value="Update"/>
